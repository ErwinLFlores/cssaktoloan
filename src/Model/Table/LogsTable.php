...
class LogsTable extends Table
{
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ->scalar('log_controller')
            ->maxLength('log_controller', 255)
            ->requirePresence('log_controller', 'create')
            ->notEmptyString('log_controller');

        $validator
            ->scalar('log_action')
            ->maxLength('log_action', 255)
            ->requirePresence('log_action', 'create')
            ->notEmptyString('log_action');
