...
class ResidentsController extends AppController
{
    ...
    {
        $this->set('page_title', 'Manage Residents Data');
        $search_by_family = $this->Categories->findByMergeValue('search_by_residents');
        $isPaginated = true;
        ...
        if (
            ...
            $this->loggerIt('Residents', 'search', $this->Auth->user('id'), $searched_conditions, 'Searched Data');
        }
        ...
        $families = $this->paginate('Residents');
    ...
    {
        $this->set('page_title', 'Residents Members Directory');
        $search_by_members = $this->Categories->findByMergeValue('search_by_resident_members');
        $isPaginated = true;
        $searched_data = $this->request->getQuery('search');
        $searched_column = $this->request->getQuery('scolumn');
        $searched_conditions = "";
        ...
        if (
            ...
            $searched_conditions = $searched_column . " LIKE '%" . $searched_data . "%'";
            $this->loggerIt('ResidentMembers', 'search', $this->Auth->user('id'), $searched_conditions, 'Searched Data');
        }
        ...
        $families = $this->paginate('ResidentMembers');
