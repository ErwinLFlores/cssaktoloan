            ...
            <div class="card">
                ...
                <div class="card-content">
                    <div style="padding:10px;">
                        Registered SAP Beneficiary Family Members
                    </div>
                        ...
                        <table class="table table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:10%;">Health Condition</th>
                            ...
                            <tbody>
                                <?php if (count($families) > 0): ?>
                                    <?php foreach ($families as $family): ?>
                                        <tr>
                                            <td><?=h($family->family_serial_key);?></td>
                                            <td><?=h($family->fullname);?></td>
                                            <td><?=h($family->relation);?></td>
                                            <td><?=h($family->gender);?></td>
                                            <td><?=h($family->birthdate->format('Y-M-d'));?></td>
                                            <td><?=h($family->work);?></td>
                                            <td><?=h($family->sector);?></td>
                                            <td><?=h($family->health_condition);?></td>
                                            <td style="text-align:left;">
                                                <?php 
                                                    if (
                                                        (isset($logged_roles['Sap']['action_index']))
                                                        && ($logged_roles['Sap']['action_index'] === 1)
                                                    ) {  ?>
                                                        <?= $this->Form->create('Add Search Item', ['type' => 'get', 'url' => ['controller' => 'Sap', 'action' => 'index', 1]]); ?>
                                                            <input type="hidden" value="family_serial" name="scolumn"/>
                                                            <input type="hidden" value="<?=h($family->family_serial_key);?>" name="search"/>
