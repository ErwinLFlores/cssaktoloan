            ...
            <div class="card">
                <div class="card-header">
                    <div class="col-sm-6 no-padding">
                        ...
                        <?php 
                            if (
                                (isset($logged_roles['Sap']['action_members']))
                                && ($logged_roles['Sap']['action_members'] === 1)
                                && (isset($member_search))
                                && ($member_search == 1)
                                && (isset($searched_column))
                                && ($searched_column == 'family_serial')
                                && (isset($searched_data))
                                && (!empty($searched_data))
                            ) { ?>
                                <?= $this->Form->create('Add Search Item', ['type' => 'get', 'url' => ['controller' => 'Sap', 'action' => 'members']]); ?>
                                    <input type="hidden" value="family_serial_key" name="scolumn"/>
                                ...
                                <?php if (count($families) > 0): ?>
                                    <?php foreach ($families as $family): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <img width="150px" src="<?=h((isset($member_search)) ? '/' : '');?><?=h($family->picture);?>" alt="User Photo">
                                            </td>
                                            <td><?=h($family->fullname);?></td>
                                            <td><?=h(ucwords($family->gender));?></td>
                                            <td><?=h(ucwords($family->civil_status));?></td>
                                            <td><?=h($family->birthdate->format('Y-M-d'));?></td>
                                            <td><?=h($family->family_serial);?></td>
                                            <td style="text-align:left;">
                                                <?php 
                                                    if (
                                                        (isset($logged_roles['Sap']['action_view']))
                                                        && ($logged_roles['Sap']['action_view'] === 1)
                                                    ) {  ?>
                                                        <?=$this->Html->link($this->Html->tag('i', '', ['class'=>'fa fa-eye']), 
                                                            ['action' => 'view', $family->family_serial], ['class' => '', 'style' => 'color:orange;', 'escape' => false]); ?>
                                                <?php } ?>

                                                <?php 
                                                    if (
                                                        (isset($logged_roles['Sap']['action_edit']))
                                                        && ($logged_roles['Sap']['action_edit'] === 1)
                                                    ) {  ?>
                                                        <?=$this->Html->link($this->Html->tag('i', '', ['class'=>'fa fa-pencil-square-o']), 
                                                            ['action' => 'edit', $family->family_serial], ['class' => '', 'style' => 'padding-left:10px', 'escape' => false]); ?>
                                                <?php } ?>

                                                <?php 
                                                    if (
                                                        (isset($logged_roles['Sap']['action_delete']))
                                                        && ($logged_roles['Sap']['action_delete'] === 1)
                                                        && (false)
                                                    ) {  ?>
