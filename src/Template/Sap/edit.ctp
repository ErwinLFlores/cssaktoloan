...
<p>
    <?=$this->Html->link('[Return] SAP Manage', ['controller' => 'sap', 'action' => 'index']) ?> > Edit
</p>
<div class="container-fluid" style="background-color:white;">
    <?= $this->Form->create('Register SAP', ['type' => 'post', 'url' => '/sap/edit/' . $sap_serial, 'enctype' => "multipart/form-data"]); ?>
        <div class="row ">
                ...
                <div id="step-11">
                    <h2 class="StepTitle">
                        <div class="col-md-12 col-xs-12">
                            Step 1 of 5 - <b>Personal Information</b>
                        </div>    
                    </h2>
                    <div class="form-group row">
                        ...
                        <div class="col-md-6 col-sm-6">
                            <input name="fullname" type="text" required="required" 
                                value="<?=h($data->fullname);?>" placeholder="LASTNAME, FIRSTNAME MIDDLENAME "
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6 col-sm-6">
                            <input name="birthdate" class="form-control" 
                                value="<?=h($data->birthdate->format('Y-m-d'));?>"
                ...
                <div id="step-44">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6 col-sm-12">
                            <img id="img_picture" src="/<?=h((!empty($data->picture))? $data->picture : 'SAP/srcs/user.jpg');?>" 
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6 col-sm-12">
                            <img id="img_id_picture" src="/<?=h((!empty($data->id_picture))? $data->id_picture : 'SAP/srcs/id_picture_sample.jpg');?>" 
                    ...
                    <div class="col-md-12 no-padding">
                        <table id="member_table_add" class="table table-responsive table-striped">
                            ...
                            <tbody id="row_container">
                                <?php 
                                    $for_counter = 0;                                     
                                    foreach ($fam_members as $key => $value) { $for_counter += 1; ?>
                                        <tr class="canDeletable">
                                            <td class="td_fullname">
                                                <input type="text" value="<?=h($value->fullname);?>" readonly 
                                                    name="family_member[<?=h($for_counter);?>][fullname]" 
                                                    style="border-color: transparent;height: 26px;background: transparent;box-shadow: 
                                                        none;border-color: transparent;font-size: 9px; padding: 0px;margin: 0px;">
                                            </td>
                                            <td class="td_relation">
                                                <input type="text" value="<?=h($value->relation);?>" readonly n
                                                    name="family_member[<?=h($for_counter);?>][relation]" 
                                                    style="border-color: transparent;height: 26px;background: transparent;box-shadow: 
                                                        none;border-color: transparent;font-size: 9px; padding: 0px;margin: 0px;">
                                            </td>
                                            <td class="td_birthdate">
                                                <input type="text" value="<?=h($value->birthdate->format('Y-m-d'));?>" readonly 
                                                    name="family_member[<?=h($for_counter);?>][birthdate]" 
                                                        style="border-color: transparent;height: 26px;background: transparent;box-shadow: 
                                                            none;border-color: transparent;font-size: 9px; padding: 0px;margin: 0px;">
                                            </td>
                                            <td class="td_gender">
                                                <input type="text" value="<?=h($value->gender);?>" readonly 
                                                    name="family_member[<?=h($for_counter);?>][gender]" 
                                                        style="border-color: transparent;height: 26px;background: transparent;box-shadow: 
                                                            none;border-color: transparent;font-size: 9px; padding: 0px;margin: 0px;">
                                            </td>
                                            <td class="td_work">
                                                <input type="text" value="<?=h($value->work);?>" readonly 
                                                    name="family_member[<?=h($for_counter);?>][work]" 
                                                    style="border-color: transparent;height: 26px;background: transparent;box-shadow: 
                                                        none;border-color: transparent;font-size: 9px; padding: 0px;margin: 0px;">
                                            </td> 
                                            <td class="td_sector">
                                                <input type="text" value="<?=h($value->sector);?>" readonly 
                                                    name="family_member[<?=h($for_counter);?>][sector]" 
                                                    style="border-color: transparent;height: 26px;background: transparent;box-shadow: 
                                                        none;border-color: transparent;font-size: 9px; padding: 0px;margin: 0px;">
                                            </td>
                                            <td class="td_health_condition">
                                                <input type="text" value="<?=h($value->health_condition);?>" readonly 
                                                    name="family_member[<?=h($for_counter);?>][health_condition]" 
                                                    style="border-color: transparent;height: 26px;background: transparent;box-shadow: 
                                                        none;border-color: transparent;font-size: 9px; padding: 0px;margin: 0px;">
                                            </td>
                                            <td class="td_action">
                                                <button type="button" class="this_delete no-padding" 
                                                    class="this_delete no-padding" style="background-color:transparent">
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#gender').val('<?=h($data->gender);?>').trigger('change');
        $('#civil_status').val('<?=h($data->civil_status);?>').trigger('change');
        $('#house_type').val('<?=h($data->house_type);?>').trigger('change');
        $('#beneficiary').val('<?=h($data->beneficiary);?>').trigger('change');
        $('#sector').val('<?=h($data->sector);?>').trigger('change');
        $('#health_condition').val('<?=h($data->health_condition);?>').trigger('change');
